...
const FormularioPage = ({ searchParams }: NextPage) => {
    ...
    try {
      if (!id) {
        ...
        const urls = await Promise.all([
          uploadFile("/fed", electionMinutes, "electionMinutes"),
          uploadFile("/fed", federationDocument, "federationDocument"),
          uploadFile("/fed", logo, "logo"),
          uploadFile("/fed", presidentDocument, "presidentDocument"),
        ]);
        ...
        const newUrls = await Promise.all([
          ...
          federationDocument
            ? uploadFile("/fed", federationDocument, "federationDocument")
            : null,
          logo ? uploadFile("/fed", logo, "logo") : null,
          presidentDocument
            ? uploadFile("/fed", presidentDocument, "presidentDocument")
            : null,
        ]);
  ...
  const isSubmitting = createStatus === "loading" || isLoadingUpload;
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="col-span-1">
            <Textfield label="Estado" id="uf" {...register("uf")} />
          </div>
          <div className="col-span-2">
